...
#### Q: I added a file/folder to `.gitignore` but it is still showing up or being pushed to Git. Why?
**A**: `.gitignore` only prevents untracked files from being added to Git. If a file or folder was already tracked and committed before being added to the gitignore, Git will continue tracking it. To stop tracking it without deleting it from your disk, remove it from the Git index:
```bash
